...
FACE_DETECTOR = mp.solutions.face_mesh.FaceMesh(
    refine_landmarks=True, min_detection_confidence=0.8, min_tracking_confidence=0.5, max_num_faces=3
)
...
def video_frame_callback(frame):
    ...
    if len(matches) == 0:
        ...
        name = matches[0].gallery_id.name

    # Draw annotations
    # frame, name = draw_annotations(frame, detections, matches)
...
for file in files:
    ...
    if detections:
        ...
        gallery.append(
            Identity(name= file.split('.')[0], embedding=subjects[0].embedding, face=subjects[0].face)
        )